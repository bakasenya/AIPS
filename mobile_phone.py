...
def load_darknet_weights(model, weights_file):
    ...
    weight_f = open(weights_file, 'rb')
    major, minor, revision, seen, _ = np.fromfile(weight_f, dtype=np.int32, count=5)
    ...
    for name in layers:
        sub_model1 = model.get_layer(name)
        for idx, layer in enumerate(sub_model1.layers):
            if not layer.name.startswith('conv2d'):
                continue
                
            #Handles the special, custom Batch normalization layer
            batch_norm_layer = None
            if idx + 1 < len(sub_model1.layers) and \
                    sub_model1.layers[idx + 1].name.startswith('batch_norm'):
                batch_norm_layer = sub_model1.layers[idx + 1]
            ...
            if batch_norm_layer is None:
                conv_bias = np.fromfile(weight_f, dtype=np.float32, count=filters)
            else:
                # darknet [beta, gamma, mean, variance]
                bn_weights = np.fromfile(
                    weight_f, dtype=np.float32, count=4 * filters)
            ...
            conv_weights = np.fromfile(
                weight_f, dtype=np.float32, count=np.product(conv_shape))
    ...
    assert len(weight_f.read()) == 0, 'failed to read all data'
    weight_f.close()
